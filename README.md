...
| `prd-spec-author` | Write or incrementally refine PiFlow's shared PRD source document `output-stages/prd/prd-spec.md` from structured requirement truth while preserving existing non-empty sections, client targets, feature tables, scope boundaries, and deployment architecture wording. | `skills/prd-spec-author/SKILL.md`, `skills/prd-spec-author/agents/openai.yaml` |
| `prd-client-author` | Generate one PiFlow per-client PRD JSON and matching `feature_list-<client_target>.md` from `output-stages/prd/prd-spec.md`, keeping target-specific schema fields, feature IDs, scope, and completeness aligned. | `skills/prd-client-author/SKILL.md`, `skills/prd-client-author/agents/openai.yaml` |
| `prd-reviewer` | Review one PiFlow per-client PRD output and produce canonical `output-stages/prd-review/prd-review-<client_target>.json` results with blocking issues, feature assessments, clarification candidates, and readiness scores before design. | `skills/prd-reviewer/SKILL.md`, `skills/prd-reviewer/agents/openai.yaml` |
