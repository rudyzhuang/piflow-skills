...
- 保留需求追溯字段：`requirement_id`、`item_id`、`source_item_id`、`version_number`、`version_hash`、`version_status`。
- 支持 AI 评审后的 freeform 一对多拆分结果：导出派生 structured 清单项，并排除原 freeform 来源项，避免重复进入 `req.md`。
  ...
  "features": [
    {
      ...
      "source_item_id": "freeform-source-item-uuid-or-null",
      "version_number": 3,
  ...
  "test_cases": [
    {
      ...
      "structured_source": "ai",
      "freeform_content": "可选测试用例业务原文，仅用于追溯或测试步骤兜底。",
      "freeform_source": "user"
    }
  ],
  "non_functional": {
...
- `features[]` 只包含当前版本未软删、且不是“已拆分原 freeform 来源项”的 `feature` 清单项。
- 如果某条 freeform 清单项已被 AI 拆分为多条 structured 派生项，原 freeform 来源项不得出现在 `features[]` 中。
- AI 派生的 Feature 必须作为普通 Feature 渲染，同时通过 `source_item_id` 追溯到原 freeform 清单项。
...
- `requirement_id`
- `item_id`
- `source_item_id`，非派生清单项允许为 `null` 或空字符串
- `version_number`
- `version_hash`
- `version_status`
- `priority`
...
- `structured_source`: `user | ai`
- `freeform_source`: `user | ai`
...
- `source_item_id` 不直接改变渲染内容，但必须保留在 Feature 块和 `acceptance_criteria` 的追溯字段中；非派生清单项可输出为空。
- `freeform_content` 不直接作为独立章节渲染；仅在 `description` 缺失时作为兜底。
- `structured_content` 是模板字段的主来源；`structured_source` 和 `freeform_source` 只用于追溯，不作为业务字段渲染。
...
- `test_cases[]` 只包含当前版本未软删、且不是“已拆分原 freeform 来源项”的 `test_case` 清单项。
- 如果 freeform 来源项被拆分为多个测试用例，原来源项不得出现在 `test_cases[]` 中，派生测试用例必须保留 `source_item_id`。
...
4. 结构化数据校验。
   ...
   - 校验 `features[]` 和 `test_cases[]` 不包含已被 AI 拆分的原 freeform 来源项。
   - 校验派生项的 `source_item_id` 指向原 freeform 清单项。
   - 校验追溯字段完整。
...
7. 输出前自检。
   ...
   - 检查每个 Feature 有 `requirement_id`、`item_id`、`source_item_id`、`version_number`、`version_hash`、`version_status`。
   - 检查派生 Feature 和 Test Case 均保留 `source_item_id`，并且原 freeform 来源项未被重复渲染。
   - 检查每个 Test Case 有 `item_id`、`source_item_id`、`version_number`、`version_hash`、`version_status`。
...
每个来自 Backend 需求项的 Test Case 必须包含：
...
- 输出 Feature 块保留版本追溯字段，包括 `source_item_id`。
- 当 Backend 返回 AI 拆分后的派生 structured 清单项时，只渲染派生项和未拆分普通项，不重复渲染原 freeform 来源项。
