...
字段说明：
- feature_id：已有功能请保留；新功能请按 PiFlow 规则自动生成，不要留空。
  - 格式：全大写短横线分段并以三位序号结尾，例如 AUTH-LOGIN-001。
  - 单端功能用端前缀：website/web/frontend -> WEB，admin -> ADMIN，backend/api -> BACKEND，mobile/ios/android -> MOB。
  - 多端功能用业务领域前缀，不要用端前缀，例如 AUTH-LOGIN-001、NOTE-CRUD-001、ORDER-CHECKOUT-001。
  - 同一前缀/动作族从 001 递增；项目内必须唯一。
...
字段说明：
- feature_id：对应功能编号；能映射到某个功能时请填写该功能的 feature_id，确实无法判断时才留空。
